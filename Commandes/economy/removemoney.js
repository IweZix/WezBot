...
module.exports = {
  ...
  name: "removemoney",
  description: "Permet retirer de l'argent à un utilisateur",
  ...
  options: [
    {
      type: 'user',
      name: 'membre',
      description: "L'utilisateur débiteur",
      required: true
    }, {
      type: 'integer',
      name: 'coins',
  ...
  async run(bot, message, args) {
    ...
    try {
      ...
      if (dailyUser) {
        if (dailyUser.money < debit)
          await message.reply(`${user} n'as pas le solde nécéssaire`);
        dailys.splice(index, 1);
        money = dailyUser.money - debit;
        dailys.push({
          user_id: user.id,
          user: user.tag,
          server_id: server.id,
        ...
        await message.reply(`${user} a été débiter de ${debit} coins !\nSolde : ${money}`);
      } else {
        await message.reply({content: `${user} n'as pas le solde nécéssaire`, ephemeral: true});
      }
